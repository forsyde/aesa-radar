...
main = do
  ...
  let iSigs           = signal $ map toCubeLists $ transpose $ map (sublist (nb*nFFT)) aesaIn
      beams           = dbf iSigs
      oPC             = pc beams
  ...
  where
    ...
    dbfOutFile  args = outPath args ++ "/DBF_CS.csv"
    pcOutFile   args = outPath args ++ "/PC_CS.csv"
    dfbOutFile  args = (outPath args ++ "/DFB_R_CS.csv",outPath args ++ "/DFB_L_CS.csv")
    cfarOutFile args = (outPath args ++ "/CFAR_R_CS.csv",outPath args ++ "/CFAR_L_CS.csv")
    ---------------------------------------------------------
    -- take only the last cube of data!
    toCubeLists :: [[a]] -> Vector (Vector (Vector a))
    toCubeLists = vector . map (vector . map vector . sublist nb) 
    toListCube  = map (map fromVector . fromVector) . fromVector . last . fromSignal
    ---------------------------------------------------------
...
flags =
  ...
  ,Option ['o'] ["output"]   (ReqArg DumpPath "PATH")
    "Path to main generated file. Default: gen/AESA_OUT_CS.csv"
...
parse argv = case getOpt Permute flags argv of
               (args,_,[]) -> do
                 let select x = filter (==x) args
                     ...
                     outFile  = let p = (selArg DumpPath) in if null p then "gen/AESA_OUT_CS.csv" else p 
                 if Help `elem` args
...
time :: (NFData t) => (Signal (Vector (Vector (Vector t))))  -> IO (Double)
time y = do
    start <- getCPUTime
    -- replicateM_ lim $ do
    x <- evaluate $ y
...
instance NFData a => NFData (Signal a) where
  rnf = rnf . fromSignal
